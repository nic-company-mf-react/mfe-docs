...
* [https://github.com/orgs/nic-company-mf-react/repositories](https://github.com/orgs/nic-company-mf-react/repositories)
...
* 먼저 PC에 작업 할 폴더를 생성합니다.
  ...
  ```
* 생성한 **frontend-mf** 폴더로 이동하여 `git clone`을 실행하여 **multirepo-mfe-boilerplate** 코드를 내려 받습니다.
  ```sh
  # host 애플리케이션션 레포지토리에서 코드를 내려받습니다.
...
* 로컬 서버 띄우기
  ...
  npm run dev
  ```
  ![pnpm dev:main 예시 이미지](../assets/config/vscode_env01.png)
* **Chrome** 브라우저를 띄우고 `http://localhost:5000` 확인하면 다음과 같이 확인할 수 있습니다.(url port와 화면 결과물은 다를 수 있음.)
![pnpm dev:main 브라우저 확인 예시 이미지](../assets/config/vscode_env02.png)