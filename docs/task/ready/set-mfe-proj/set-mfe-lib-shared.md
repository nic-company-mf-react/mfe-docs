...
# mfe-lib-shared 환경구성
**@axiom/mfe-lib-shared** 는 Micro Frontend 프로젝트에서 공통으로 사용되는 라이브러리를 제공하는 **공유 라이브러리 패키지** 입니다.  
...
{ "dependencies": { "@axiom/mfe-lib-shared": "^1.0.0" } }
// npm install @axiom/mfe-lib-shared 명령으로 설치하여 사용.
```

```tsx
// 실제 사용 예시
import { Button, Card, Input, Badge, cn } from '@axiom/mfe-lib-shared/components';
import { formatDate, validateEmail } from '@axiom/mfe-lib-shared/utils';
import type { User, ApiResponse } from '@axiom/mfe-lib-shared/types';
...
import { Button, Card } from '@axiom/mfe-lib-shared/components/ui';
import { formatDate } from '@axiom/mfe-lib-shared/utils';
import type { User } from '@axiom/mfe-lib-shared/types';

// config 파일도 export 경로를 통해 사용
import eslintConfig from '@axiom/mfe-lib-shared/config/eslint';
...
* `package.json` 에 다음과 같이 관련 패키지가 설치 되어야합니다.
  ...
  - `dependencies` 필드에 설치하는 이유는 remote 앱이 npm install @axiom/mfe-lib-shared 하면 플러그인도 자동 설치되도록 하기 위해서입니다. `devDependencies` 필드에 설치하면 플러그인이 설치되지 않습니다.
  ```json
  "dependencies": {
    "@eslint/js": "^9.39.4",
    "eslint": "^9.39.4",
...
* 각 리포트 앱에서 사용할 때는 다음과 같이 사용합니다.(수정필요)
  ...
  // highlight-start
  import { react } from "@axiom/mfe-lib-shared/eslint";
  // highlight-end
...
---
* `@axiom/mfe-lib-shared` 패키지를 사용하는 앱에서 설치할 때 npm은 해당 레포지토리를 그대로 다운로드합니다. 그리고 package.json의 main, exports 필드가 모두 dist/를 가리키고 있기 때문에 `dist/`가 git에 없으면 리모트 앱에서 `npm install` 시 패키지를 찾을 수 없어 에러가 납니다. 따라서 `dist/` 폴더를 git에 추가하여 배포할 수 있도록 합니다.
...
// 개별 파일
import "@axiom/mfe-lib-shared/styles/tokens";
import "@axiom/mfe-lib-shared/styles/base";
```
:::
...
```json
{
  ...
  "dependencies": {
    "@axiom/mfe-lib-shared": "file:../..", // 공유 라이브러리 직접 참조
    "react": "^19.2.4",
    "react-dom": "^19.2.4"
  },
  "devDependencies": {
...
```json
{
  "compilerOptions": {
    ...
    "baseUrl": ".",
    "paths": {
      "@axiom/mfe-lib-shared": ["../../src/index.ts"],
      "@axiom/mfe-lib-shared/components": ["../../src/components/index.ts"],
      "@axiom/mfe-lib-shared/components/ui": ["../../src/components/shadcn/ui/index.ts"],
      "@axiom/mfe-lib-shared/styles": ["../../src/styles/index.css"],
      "@/*": ["../../src/*"] 
    }
  },
  "include": ["src"]
...
<html lang="ko">
  <head>
    ...
    />
    <title>Playground | @axiom/mfe-lib-shared</title>
  </head>
  <body>
    <div id="root"></div>
    <script
